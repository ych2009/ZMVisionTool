...
void MainWindow::on_arrowLineSBtn_clicked()
{
    m_scene.startCreateArrowLine();
    setBtnEnabled(false);
    BArrow_Line_s *m_arrow_line = new BArrow_Line_s(BGraphicsItem::ItemType::Arrow_line_s);
    m_scene.addItem(m_arrow_line);

    connect(&m_scene, SIGNAL(updatePoint(QPointF, QList<QPointF>, bool)), m_arrow_line, SLOT(pushPoint(QPointF, QList<QPointF>, bool)));
    connect(&m_scene, &BQGraphicsScene::createFinished, [=](){
        setBtnEnabled(true);
    });
}
