...
void BPointItem::mouseMoveEvent(QGraphicsSceneMouseEvent *event)
{
    if ( event->buttons() == Qt::LeftButton ) {
        ...
        case Edge: {
            ...
            case BGraphicsItem::ItemType::Arrow_line_s: {
                BArrow_Line_s *arrowline = dynamic_cast<BArrow_Line_s *>(item);
                arrowline->updatePolygon(QPointF(event->lastScenePos().x(), event->lastScenePos().y()),
                                       QPointF(event->scenePos().x(), event->scenePos().y()));
            } break;
